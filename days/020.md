...
2. Run the following command to start mlflow server according to task description:
    ...
    ```
    ```bash
    mlflow server --backend-store-uri sqlite:////root/code/mlflow-backend/mlflow.db --default-artifact-root /root/code/mlflow-artifacts/ --host 0.0.0.0 --port 5000 --cors-allowed-origins '*' --allowed-hosts '*' & 
    ```
