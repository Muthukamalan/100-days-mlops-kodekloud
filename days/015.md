...
2. Let's update `params.yaml`:
    ...
    check it's been mentioned in the `dvc.yaml`
...
4. Now, we can run `dvc repro` again:
...
5. Let's change the number in `params.yaml` from 100 to something bigger and run `dvc repro` again.
