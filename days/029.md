...
1. Update `start-mlflow.sh` with S3 Endpoint URL:
    ...
    > Make sure you are using the right endpoint with right port that is given in the problem description

2. Run the following commands:

    ```bash
    ./mlflow-restart.sh
    python3 log_test_run.py
    ```

3. Check artifacts from SeaWeedFS UI
